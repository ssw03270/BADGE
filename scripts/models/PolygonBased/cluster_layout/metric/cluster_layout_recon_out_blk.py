...
def overlap(predicted_coords, gt_coords, min_coords_outputs, range_max_outputs, save_dir):
    ...
    for idx in tqdm(range(len(predicted_coords))):
        ...
        blk_polygon = create_bounding_box(0, 0, 1, 1, 0)
        blk_polygon = denormalize_coords_uniform(blk_polygon.exterior.coords, min_coords, range_max)
        blk_polygon = Polygon(blk_polygon)

        predict_out_blk_count = 0
        for layout_idx in range(len(predicted_layouts)):
            if predicted_layouts[layout_idx].intersection(blk_polygon):
                predict_out_blk_count += 1
                 
        gt_out_blk_count = 0   
        for layout_idx in range(len(gt_layouts)):
            if gt_layouts[layout_idx].intersection(blk_polygon):
                gt_out_blk_count += 1
                    
        predicted_out_blks.append(predict_out_blk_count)
        gt_out_blks.append(gt_out_blk_count)

    # Calculate statistics
    predicted_mean = np.mean(predicted_out_blks)
    predicted_min = np.min(predicted_out_blks)
    predicted_max = np.max(predicted_out_blks)
    
    gt_mean = np.mean(gt_out_blks)
    gt_min = np.min(gt_out_blks)
    gt_max = np.max(gt_out_blks)
    
    # Print results
    print("Predicted Out Blocks - Mean:", predicted_mean, "Min:", predicted_min, "Max:", predicted_max)
    print("GT Out Blocks - Mean:", gt_mean, "Min:", gt_min, "Max:", gt_max)
    
    # Save to txt file
    os.makedirs(save_dir, exist_ok=True)
    with open(save_dir + '/result.txt', "w") as file:
        file.write("Predicted Out Blocks:\n")
        file.write(f"Mean: {predicted_mean}\nMin: {predicted_min}\nMax: {predicted_max}\n\n")
        
        file.write("GT Out Blocks:\n")
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='metric_result/out_blk', help='Directory to save the visualization plots.')
